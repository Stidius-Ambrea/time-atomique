...
// --- Récupération de l’en-tête Date ---
...
// --- Conversion HTTP Date -> timestamp ---
...
// --- Format ISO local avec fuseau horaire ---
...
(async () => {
  ...
  if (!valids.length) {
    console.error('❌ Aucune source valide détectée');
    process.exit(1);
  }
  ...
  const body = {
    ...
    status: valids.length >= 3 ? 'ok' : 'degraded'
  };
